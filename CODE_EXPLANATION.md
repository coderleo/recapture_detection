...
#### 1. Dropout (随机失活)
*   **是什么**: 一种防止过拟合的技术。在训练过程中，随机让神经网络中的一部分神经元“罢工”（输出变为0）。
*   **为什么**: 就像一个团队，如果每个人都太依赖某个“大神”，一旦大神请假，团队就垮了。Dropout 强迫每个神经元都要独立干活，不能依赖别人。这样训练出来的模型更健壮。
*   **注意**: 只有在训练 (`model.train()`) 时开启，预测 (`model.eval()`) 时所有神经元都要全力以赴。

#### 2. 前向传播 (Forward Propagation)
*   **是什么**: 数据从输入层流向输出层的过程。
*   **比喻**: 就像你做一张试卷。你看到题目（输入图片），经过大脑思考（神经网络层层计算），最后写下答案（输出预测结果）。
*   **代码对应**: `outputs = model(inputs)`

#### 3. 反向传播 (Backward Propagation)
*   **是什么**: 根据预测结果和真实答案的差距（误差），反向推导每个参数该怎么改的过程。
*   **比喻**: 老师批改试卷，发现你错了。老师会告诉你：“这道题错在公式用错了，那道题错在计算粗心了”。反向传播就是计算“每个参数对这个错误负多大责任”（即梯度）。
*   **代码对应**: `loss.backward()`

#### 4. 梯度清零 (Zero Grad)
*   **是什么**: 在进行新一轮参数更新前，把上一次计算的梯度（责任）清空。
*   **为什么**: PyTorch 设计上默认会把梯度“累加”起来（就像购物车，你不清空，新东西会叠在旧东西上）。但在训练中，我们希望每次只根据当前的图片来改参数，不受上一批图片的影响，所以每次都要手动清零。
